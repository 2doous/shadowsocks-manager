...
app.controller('AdminSettingsController', ['$scope', '$http', '$timeout', '$state',
  ...
  ($scope, $http, $timeout, $state) => {
    ...
    $scope.toSystemAccount = id => {
      $state.go('admin.paymentAccountPage', {
        accountId: id
      });
    };
    // $scope.time = [{
...
]).controller('AdminPaymentAccountPageController', ['$scope', '$http', '$timeout', '$state', '$stateParams',
  ($scope, $http, $timeout, $state, $stateParams) => {
    $scope.setTitle('套餐详情');
    $scope.setMenuButton('arrow_back', 'admin.paymentSetting');
    const accountId = $stateParams.accountId;
    $http.get(`/api/admin/systemAccount/${ accountId }`).then(console.log);
  }
])
